...
describe("Hierarchical Component Factory ", () => {
    ...
    it("should be able to add child component to one component and insert it after in the pool", () => {
        ...
        let child1 = HierarchicalFactory.createChildComponent(HierarchicalComponent, t.entityId);
        expect(HierarchicalFactory.pool[1].entityId).to.equal(child1.entityId);
        expect(HierarchicalFactory.pool[0].children.length).to.equal(1);
        expect(HierarchicalFactory.pool[0].children[0]).to.equal(child1.entityId);

        // insert a new component at the top
        let t2 = HierarchicalFactory.createComponentBefore(HierarchicalComponent, t.entityId);
        let child2 = HierarchicalFactory.createChildComponent(HierarchicalComponent, t2.entityId);

        expect(HierarchicalFactory.pool[0].entityId).to.equal(t2.entityId);
        expect(HierarchicalFactory.pool[0].children[0]).to.equal(child2.entityId);
        //child2 should be placed after it parents in the pool
        expect(HierarchicalFactory.pool[1].entityId).to.equal(child2.entityId);
        //other components should have been moved futher down the pool
        expect(HierarchicalFactory.pool[2].entityId).to.equal(t.entityId);
        expect(HierarchicalFactory.pool[3].entityId).to.equal(child1.entityId);

    });
    it("should be able to remove a parent and all its children in the factory pool", () => {
        let t = HierarchicalFactory.createComponent(HierarchicalComponent);
        let child1 = HierarchicalFactory.createChildComponent(HierarchicalComponent, t.entityId);
        let child2 = HierarchicalFactory.createChildComponent(HierarchicalComponent, t.entityId);
    ...
    it("should be able to remove the whole branch of a hierachy recursively", () => {
        ...
        let child1 = HierarchicalFactory.createChildComponent(HierarchicalComponent, t.entityId);
        let child2 = HierarchicalFactory.createChildComponent(HierarchicalComponent, t.entityId);
        let child1OfChild2 = HierarchicalFactory.createChildComponent(HierarchicalComponent, child2.entityId);
        let child2OfChild2 = HierarchicalFactory.createChildComponent(HierarchicalComponent, child2.entityId);
        let t2 = HierarchicalFactory.createComponent(HierarchicalComponent);
    ...
    it("should be able to remove a parent but not its children in the factory pool", () => {
        ...
        expect(HierarchicalFactory.pool[0].entityId).to.equal(child1.entityId);
        expect(HierarchicalFactory.pool[1].entityId).to.equal(child2.entityId);
        expect(HierarchicalFactory.pool[2].entityId).to.equal(t2.entityId);
    });
});